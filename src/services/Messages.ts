...
export const createMessage = async (data: CreateMessageArgs) => {
  ...
  const userMentionIds = extractUserMentionIds(data.content);
  const userMentions = await getUsersByIds(userMentionIds);
  const userMentionObjectIds = userMentions.map(user => user._id);

  const quoteMentionIds = extractQuoteMentionIds(data.content);
...
function extractUserMentionIds(content: string | undefined): string[] {
  if (!content) return [];
...
function extractQuoteMentionIds(content: string  | undefined): string[] {
  if (!content) return [];
